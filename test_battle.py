...
def test_take_damage():
    actor = CombatState(EltenaMaster("勇者", 20, 10, 100))
    actor.take_damage(5)
...
def test_speed_order():
    actor1 = CombatState(EltenaMaster("勇者", 20, 10, 100))
    actor2 = CombatState(EltenaMaster("スライム", 20, 5, 90))
...
def test_speed_order_same_tick_to_threshold():
    actor1 = CombatState(EltenaMaster("勇者", 20, 10, 150))
    actor2 = CombatState(EltenaMaster("スライム", 20, 5, 151))
    action = ActionOrderManager([actor1, actor2])
    action.next_actor()
...
def test_overheat_allows_double_at_3x():
    actor1 = CombatState(EltenaMaster("勇者", 200, 10, 150))
    actor2 = CombatState(EltenaMaster("スライム", 200, 5, 50))
    action = ActionOrderManager([actor1, actor2])
    action.next_actor()
...
def test_overheat_allows_double_at_4x():
    actor1 = CombatState(EltenaMaster("勇者", 200, 10, 200))
    actor2 = CombatState(EltenaMaster("スライム", 200, 5, 50))
    action = ActionOrderManager([actor1, actor2])
    action.next_actor()
...
def test_overheat_action_order():
    actor1 = CombatState(EltenaMaster("勇者", 200, 10, 60))
    actor2 = CombatState(EltenaMaster("スライム", 200, 5, 50))
    action = ActionOrderManager([actor1, actor2])
...
def test_get_enemies_player_to_enemy():
    player1 = CombatState(EltenaMaster('player1', 1, 1, 1), Camp.PLAYER)
    enemy1  = CombatState(EltenaMaster('enemy1', 1, 1, 1),  Camp.ENEMY)
    enemy2  = CombatState(EltenaMaster('enemy2', 1, 1, 1),  Camp.ENEMY)
...
def test_get_alives():
    alive1 = CombatState(EltenaMaster('alive1', 1, 1, 1))
    alive2 = CombatState(EltenaMaster('alive2', 1, 1, 1))
    death1 = CombatState(EltenaMaster('death1', 0, 1, 1))
...
def test_dead_actor_action_order():
    alive1_player = CombatState(EltenaMaster('player1', 10, 1, 1 ), Camp.PLAYER)
    alive2_enemy  = CombatState(EltenaMaster('enemy1',  10, 1, 1 ), Camp.ENEMY)
    death1_enemy  = CombatState(EltenaMaster('enemy2',  0 , 1, 10), Camp.ENEMY)
